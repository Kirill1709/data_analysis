...
class VerticalNavigationToolbar2Tk(NavigationToolbar2Tk):
   ...
   def _Spacer(self):
      s = Frame(self, width=26, relief=RIDGE, bg="White", padx=2)
...
def do_plot2():
    plt.close()
    yhat = savgol_filter(consumption, int(ent1.get()), int(ent2.get()), mode='interp')
...
def graph_1(time, consumption, temperature, color='white', color_amplitude='white', color_temperature='white', status=False):
    ...
    ax_1.set_title("График исследования амплитуды", fontsize=15, fontname='Times New Roman')
    ...
    btplot1 = Button(root, text='Подробнее', command=do_plot1)
    ...
    if max(temperature) > 100:
        ax_3.yaxis.set_major_locator(matplotlib.ticker.MultipleLocator(base=round(max(time)//10, -1)))
...
def graph_2(time, consumption, color_average='white',
    ...
    canvas_2.get_tk_widget().place(x=GRAPH_COORD_X,y=SCREEN_HEIGHT/2-50,width=SCREEN_WIDTH-GRAPH_COORD_X,height=SCREEN_HEIGHT/2+50)
    ax_2 = figure_2.add_subplot()
    ax_2.set_title("График исследования частоты", fontsize=15, fontname='Times New Roman')
    yhat = savgol_filter(consumption, int(ent1.get()), int(ent2.get()), mode='interp')
    asd=np.convolve(yhat, np.ones(int(ent3.get()))/int(ent3.get()), mode='valid')
    ...
    if max(time) > 100:
        ax_2.xaxis.set_major_locator(matplotlib.ticker.MultipleLocator(base=round(max(time)//16, -1)))
    canvas_2.draw()
    toolbar_2 = VerticalNavigationToolbar2Tk(canvas_2, root)
    toolbar_2.update()
    toolbar_2.place(x=SCREEN_WIDTH/1.05,y=SCREEN_HEIGHT/20+SCREEN_HEIGHT/2)
    toolbar_2.config(background='white')
    btplot2 = Button(root, text='Подробнее', command=do_plot2)
    btplot2.place(x=SCREEN_WIDTH/1.07, y=SCREEN_HEIGHT-SCREEN_HEIGHT/10, width=80, height=30)
    canvas_2.mpl_connect('button_press_event', onclick)
    try:
        ax_2.fill_between([time[section[0]], time[section[1]]], min(yhat), max(yhat), color='green', alpha=0.2)
    except Exception:
        pass
...
def onclick(event):
    ...
    if len(ax_2.lines) == 4:
        line_1 = ax_2.lines[2]
        line_2 = ax_2.lines[3]
        if abs(line_1.get_xdata()[0]-x_data) <= abs(line_2.get_xdata()[0]-x_data):
            if is_left():
                ent4.delete(0, END)
...
def binarySearch(arr, x, left, right, l_point, r_point):
    if right <= left:
        graph_2(time, consumption, 'red', 'blue', 'black', True)
        return messagebox.showerror('Ошибка', 'Такого значения не найдено')
    mid = (left + right) // 2
...
def freq_moving_user(ax_2, time, asd, yhat, color, status):
    if status:
        ...
        begin = left = binarySearch(time, float(ent4.get()), 0, len(time)-1, 0.98, 1)
        right = binarySearch(time, float(ent5.get()), 0, len(time)-1, 1, 1.02)
        ax_2.plot([time[begin], time[begin]], [min(yhat), max(yhat)], color=color)
        ax_2.plot([time[right], time[right]], [min(yhat), max(yhat)], color=color)
    list_av = []
    freq_list = []
    while left <= right:
        if  yhat[left] < asd[left] < yhat[left+1] or yhat[left] > asd[left] > yhat[left+1]:
    ...
    if len(freq_list) == 0:
        ...
        ax_2.text(time[((begin+right)//2)], max(yhat), f'{freque} Гц', fontsize=9, color='black', horizontalalignment='center')
...
btplot5 = Button(root, text='Перестроить', command=rebuild)
...
ent2 = Entry(root, bd=2)
ent2.place(x=20, y=130, width=45, height=25)
ent3 = Entry(root, bd=2)
ent3.place(x=20, y=160, width=45, height=25)
ent4 = Entry(root, bd=2)
ent4.place(x=20, y=360, width=45, height=25)
ent4.insert(0, '0')
ent5 = Entry(root, bd=2)
ent5.place(x=100, y=360, width=45, height=25)
ent5.insert(0, '0')
ent6 = Entry(root, bd=2)
ent6.place(x=20, y=500, width=45, height=25)
ent6.insert(0, '10')
lab1 = Label(root, text='Период сглаживания', font='Times 12')
lab1.place(x=70, y=100)
lab1.configure(bg='#EEE9E9')
lab2 = Label(root, text='Порядок полинома', font='Times 12')
lab2.place(x=70, y=130)
lab2.configure(bg='#EEE9E9')
lab3 = Label(root, text='Период скользящей средней', font='Times 12')
lab3.place(x=70, y=160)
lab3.configure(bg='#EEE9E9')
lab4 = Label(root, text='Представление сигнала в частотной области', font='Times 13', wraplength=250)
lab4.place(x=20, y=240)
lab4.configure(bg='#EEE9E9')
lab5 = Label(root, text='Параметры графика', font='Times 13')
lab5.place(x=20, y=60)
lab5.configure(bg='#EEE9E9')
lab6 = Label(root, text='Вычисление частоты отрезка', font='Times 13')
lab6.place(x=20, y=330)
lab6.configure(bg='#EEE9E9')
lab7 = Label(root, text='Отклонение амплитуды от средней', font='Times 13')
lab7.place(x=20, y=450)
lab7.configure(bg='#EEE9E9')
lab8 = Label(root, text='%', font='Times 13')
